 ...
 * @version V1.0.0
 */
@Component("accountSpecialDebitFacade")
public class AccountSpecialTallyDebitFacadeImpl implements AccountTallyFacade {

	private static Logger logger = LoggerFactory.getLogger(AccountSpecialTallyDebitFacadeImpl.class);
	...
	public void tally(TradeVoucher tradeVoucher, AccountBussinessInterfaceBean accountBussinessInterfaceBean) {
		try {
			...
			logger.info("账务系统接收业务系统请求[特殊出账]指令：{}", JsonUtils.toJsonString(tallyVoucher));

			if (tallyVoucher instanceof AccountSpecialTransitVoucher) { // Capital account types transit
				...
				AccountSpecialTransitVoucher accountTransitVoucher = (AccountSpecialTransitVoucher) tallyVoucher;
				if (accountTransitVoucher.getTransitBalance() == null) throw new BussinessException(ExceptionMessages.PARAM_NOT_EXISTS);

				AccountTransitSummary valuelessTransitSummary = accountTransitSummaryService.findTransitSummaryBy(account.getCode(), null);
				...
				if (tallyVoucher.getWaitAccountDate() == null) {
					...
					List<AccountTransitSummary> accountTransitSummarys = accountTransitSummaryService.findBatchTransitSummaryBySeq(account.getCode(),
					...
					AccountTransitSummary fallbackSummary = accountTransitSummaryService.findTransitSummaryBy(account.getCode(),
				...
				accountRecordedDetailService.create(account.getCode(), tallyVoucher.getUserNo(), tallyVoucher.getUserRole(),
						accountBussinessInterfaceBean.getSystemFlowId(), accountBussinessInterfaceBean.getBussinessCode(), tallyVoucher.getCurrency(),
						tallyVoucher.getTransOrder(), tallyVoucher.getTransDate(), accountBussinessInterfaceBean.getSystemCode(), tallyVoucher.getWaitAccountDate(),
			...
			mqSendMessage.send(tradeVoucher,tallyVoucher.getUserNo(),accountBussinessInterfaceBean.getBussinessCode());
//			try{	
//				SimpleDateFormat df=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
//				Map<String, Object> map=new HashMap<String, Object>();
//				map.put("accountNo", tallyVoucher.getUserNo());
//				map.put("currency", tallyVoucher.getCurrency());
//				map.put("transOrder",tallyVoucher.getTransOrder().toString());
...
//				logger.info("推送主题 yl-data-topic,标签：账户流水  成功，商户编号{}，消息体{}", tallyVoucher.getUserNo(),map);
//			} catch (Exception e) {
