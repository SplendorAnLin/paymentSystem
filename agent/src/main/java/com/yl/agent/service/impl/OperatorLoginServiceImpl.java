...
import com.yl.agent.entity.MenuComparator;
...
import com.yl.agent.utils.HttpGetJson;
import com.yl.agent.utils.IpUtil;
...
//			}
			...
			auth.setAgentNo(operator.getAgentNo());
		...
		for (Role role : roles) {
			if (role.getStatus() == Status.TRUE) {
				...
				for (Function function : functions) {
					if (function.getStatus() == JsonUtils.toJsonToObject(Status.TRUE, com.yl.agent.api.enums.Status.class)) {
						fun.add(function);
					}
				}
			}
