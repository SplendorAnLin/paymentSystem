...
<body style="min-width: 1000px; padding: 0.2em;">
            ...
            <div class="item">
              <div class="input-area">
                ...
                <div class="input-wrap">
                  <input type="text" class="input-text" name="role.name" value="${role.name }" required checkRoleName trigger='{"checkRoleName": 2}'>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="input-area">
                ...
                <div class="input-wrap">
                  <textarea name="role.remark" cols="50" rows="8" required >${role.remark }</textarea>
                </div>
              </div>
            </div>
          </div>
              ...
              <div class="row">
                ...
                <div class="primary fix">
                  ...
                  <div class="check-list">
                    ...
                    <input type="checkbox" class="hidden" name="menuList[${rootStatus.index }]" value="${menu.id }">
                  </div>
                  <!-- 一级右侧 -->
                  <div class="sub">
                          ...
                          <div class="secondary-row">
                            ...
                            <div class="check-list">
                              <input type="checkbox" class="checkbox"  value="${childMenu.id }" name="menuList[${childStatus.index }]" 
                                <c:forEach items="${roleMenu }" var="roleInfoC">
                                  <c:if test="${roleInfoC.id==childMenu.id }">checked</c:if>
                                </c:forEach>     
                              ">
                                ...
                                <c:if test="${function.menuId==childMenu.id}">
                                  <span class="featur">
                                    <input type="checkbox" class="checkbox" name="functionList[${funI.index }]" value="${function.id }"
                                      <c:forEach items="${roleFunction }" var="roleInfoF">
                                        <c:if  test="${roleInfoF.id==function.id }">checked</c:if>
                                      </c:forEach>
                                    >
  ...
  <script>
    ...
    Compared.add('checkRoleName', function(val, params, ele, ansyFn) {
      ...
      Api.agent.checkRoleName(val, function(status) {
...
/*     $('form').data('receiver-hook', function(status, closeDialog, iframe) {
      ...
      closeDialog(msg == true || msg == 'true', null, true);
    }); */
