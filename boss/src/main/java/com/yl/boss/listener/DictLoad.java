...
public class DictLoad implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent sce) {
		...
		try {
			...
			com.yl.utils.Constant.DICTS= JsonUtils.toObject(JsonUtils.toJsonString(Constant.DICTS), new TypeReference <Map <String, com.alibaba.fastjson.JSONObject>>() {
			});
			logger.info("字典初始化成功,用时:" + (System.currentTimeMillis() - startTime)+ "ms");
		} catch (Exception e) {
			logger.error("字典初始化异常:{}",e);
			try {
				MailUtils.send(MailType.EXCEPTION, "【运营系统】字典加载异常",e.getMessage(), prop.getProperty("mail.from"));
