...
import com.yl.boss.entity.MenuComparator;
...
import com.yl.boss.utils.HttpGetJson;
import com.yl.boss.utils.IpUtil;
...
public class OperatorLoginServiceImpl implements OperatorLoginService {
	...
	private static final Logger logger = LoggerFactory.getLogger(OperatorLoginServiceImpl.class);
	...
	// 系统登录
	public Authorization login(String username, String password, String ipAddress, String sessionId)
...
//        if (operator.getRoles() == null || operator.getRoles().size() <= 0) {
        ...
        operator.getRoles().add(roleDao.findById(operator.getRoleId()));
//        }
...
//			 String endTime = operator.getAllowEndTime();
			 ...
			 if((now - lastly) < intervalMinute*60000 && times >
			 (permitLoginTimes-1)){
...
//			 if(!DateUtil.CompareDays(pwdModifyTime,new
//			 Date(),ModifyPasswdCycle)){
...
//			 }
			...
			auth.setAuditPassword(operator.getAuditPassword());
			auth.setUsePhoneCheck(operator.getUsePhoneCheck());
			return auth;
		} else {
			Integer times = operator.getMaxErrorTimes();
			Date lastLoginErrTime = operator.getLastLoginErrTime();
			...
			if (lastLoginErrTime != null) {
			 ...
			 createLoginLog(username,ipAddress,LoginStatus.LOCK,sessionId);
			 throw new
			 LoginException("您已经连续登陆失败"+permitLoginTimes+"次请在"+intervalMinute+"分钟后再次登陆");
			 }
	...
	// 登录成功后的信息加载
	private Authorization loginSucess(Operator operator, String ipAddress, LoginLog loginLog) {
		operator.setMaxErrorTimes(0);
		operator.setLastLoginErrTime(null);
		operator = operatorDao.update(operator);

		Authorization auth = getAuthorization(operator, ipAddress, loginLog.getId());
		return auth;
	}

	// 生成认证信息
	private Authorization getAuthorization(Operator operator, String ipAddress, Long loginLogId) {
	...
	// 创建登录日志
	public LoginLog createLoginLog(String username, String requestIp, LoginStatus loginStatus, String sessionId) {
		Date date = new Date();
//		if (LoginStatus.SUCCESS.equals(loginStatus)) {
			// 查找同一个用户名的前一次成功并且没有退出的登录日志，设置logouttime，即将该session做被迫登出处理
//			 List<LoginLog> loginLogs = loginLogDao.findByUsernameAndStatusAndLogoutTimeNull(username, loginStatus);
//			 if(loginLogs.size()>0){
