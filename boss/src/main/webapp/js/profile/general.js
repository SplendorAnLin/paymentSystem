...
(function() {
  ...
  general.addAgentVerify = function() {
    ...
    Compared.add('checkagentPhone', function(val, params, ele, ansyFn) {
      // 服务商和商户公用这个API
      Api.boss.checkAgentPhone(val,function(exist) {
    ...
    Compared.add('checkAgentFullName', function(val, params, ele, ansyFn) {
      Api.boss.checkAgentFullName(val,function(fullName) {
    ...
    Compared.add('checkAgentSortName', function(val, params, ele, ansyFn) {
      if (val == lastSortName && val != '')
        return Compared.toStatus(true);
      Api.boss.checkAgentSortName(val,function(fullName) {
    ...
    // mcc验证
    var lastMcc = $('.mcc').val();
    Compared.add('mcc', function(val, params, ele, ansyFn) {
      if (lastMcc == val && val != '')
        return Compared.toStatus(true);
      Api.boss.checkMcc(val,function(msg) {
        ansyFn(Compared.toStatus(msg != 'false', 'mcc不存在'));
      });
    });

    // 根究所属服务商编号切换验证
    general.feesForParent = function(parentNo, callback) {
      if (parentNo == '') {
        verifyForDefault();
        callback(false);
        return;
      }
      Api.boss.findAgentFees(parentNo, function(fees) {
    ...
    function verifyForParent() {
      ...
      utils.each(window.parentFeesInfo, function(fee) {
        ...
        //   return;        // if (productItem.attr('disabled'))
        //   return;
  ...
  general.addCustomerVerify = function() {
    ...
    Compared.add('checkCustomerSortName', function(val, params, ele, ansyFn) {
      if (val == lastSortName && val != '')
        return Compared.toStatus(true);
      Api.boss.checkCustomerSortName(val,function(fullName) {
  ...
  general.infaceInfoSelect = function(infaceType, select) {
    // 查询接口信息, 并填入下拉列表
    Api.boss.interfaceInfoCodeQueryBy(infaceType, function(interfaceInfo) {
      if (!interfaceInfo)
        return;
    ...
    Compared.add('checkPosCustomerNo', function(val, params, ele, ansyFn) {
      ...
      utils.ajax({
    	  ...
    	  success: function(customerInfo) {
    		  ...
    		  } else if (customerInfo == 'superior') {
                  ansyFn(Compared.toStatus(false, '商户已存在上级代理商!'));
                  customerNameInput.val('');
    		  } else {
