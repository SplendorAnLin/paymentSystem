              ...
              <td>
                ...
                <c:if test="${acc.status.name() != 'FREEZE'}">
                  <button data-myiframe='{
                    "target": "toAccountAdjustA.action?id=${acc.code }&&balance=${acc.balance }",
                    "btns": [
                      {"lable": "取消"},
                      {"lable": "调帐", "trigger": ".btn-submit"}
                    ]
                  }' class="btn-small mr-5">调帐</button>
                </c:if>
                <button data-dialog="accountHistoryAction.action?accountNo=${acc.code }" class="btn-small">调帐历史</button>
              </td>
            </tr>
          </c:forEach>
        </tbody>
