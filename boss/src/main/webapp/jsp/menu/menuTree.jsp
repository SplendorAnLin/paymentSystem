...
<body style="min-width: 1150px;">
  ...
  <div class="fix pd-10">
    ...
    <!-- 表单 -->
    <form id="tree-form" action="toMenuEdit.action" method="post" target="menuDetail">
  ...
  <c:forEach items="${menuList}" var="m">
    ...
    <c:forEach items="${menuList}" var="cm">
      <c:if test="${m.id==cm.parentId }">
        <script>
          if ('${cm.id}' != '${cm.parentId}') {
          	menuTree.add("${cm.name}", "${cm.id}", "${cm.parentId}");
          }
        </script>
      </c:if>
    </c:forEach>
