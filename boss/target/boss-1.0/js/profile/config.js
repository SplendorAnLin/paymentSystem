  ...
  if (window.top === window) {
    ...
    MyDialog.open = function(dialog, content) {
      ...
      if (typeof(dialog.options.target) != 'string') {
        // 清空表单
        $(content[0]).removeClass('not-ready-render');
        $(content[0]).find('.not-ready-render').removeClass('not-ready-render');
        // 重新渲染组件
...
(function() {
  ...
  $.fn.dictCitySelect = function(opt) {
    ...
    // 初始化获取所有省
    Api.boss.queryProvList(function(provList) {
      if (provList == null) {
    ...
    function changeProv(provcode) {
      ...
      Api.boss.queryCityList(provCode, function(cityList) {
        if (cityList == null) {
