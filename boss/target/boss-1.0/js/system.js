...
(function() {
  ...
  // 拷贝菜单列表到抽屉菜单栏
  $('#drawer-menu-list').html($('#menu-list').html());
...
(function() {
  ...
  WindowManage.prototype.create = function(url, title, notload) {
    ...
    if (this.checkUrl(url)) {
      ...
      if (i == -1) {
        var index = this.getIndexForOrigin(url);
        this.windows[index].toOrgin();
        return;
      }
  ...
  // 根据原始URL获取索引
  WindowManage.prototype.getIndexForOrigin = function(url) {
    var self = this;
    var index = -1;
    $.each(self.windows, function(i) {
      if (this.origin == url) {
  ...
  // 跳转原始
  Windows.prototype.toOrgin = function() {
    windowManage.loading(false);
    this.title = this.originName;
    this.iframe.attr('src', this.origin);
  };
