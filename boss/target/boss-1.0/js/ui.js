...
window.query = function() {
  ...
  var btn = $('.query-btn'); 
  var show_viewCount = $('#show_viewCount');
  console.log(show_viewCount)
  ...
  $('.query-form form').submit(function(event) {
    ...
    show_viewCount.addClass('disabled', 'disabled');
    btn.loading(10000, false, function() {
    	show_viewCount.removeAttr('disabled');
  ...
  utils.listenIframe(iframe, function(status) {
    btn.ending();
    show_viewCount.removeClass('disabled');
    resultIframeWrap.hideLoadImage();
    if (!status) {
      return;
    }
