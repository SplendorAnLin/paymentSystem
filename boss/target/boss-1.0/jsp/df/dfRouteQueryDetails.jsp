          ...
          <div class="adaptive fix">
            ...
            <div class="item">
              <div class="input-area">
                ...
                <div class="input-wrap">
                  <input type="text" class="input-text" name="routeConfigBean.routeInfo.priority" bled" value="${routeConfigBean.routeInfo.priority }" disabled>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="input-area">
                ...
                <div class="input-wrap">
                  <select class="input-select" name="routeConfigBean.routeInfo.remitCode" data-defaultValue="${routeConfigBean.routeInfo.remitCode }" disabled>
                  </select>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="input-area">
                ...
                <div class="input-wrap">
                  <dict:select styleClass="input-select" value="${routeConfigBean.status }" dictTypeId="STATUS" disabled="true"></dict:select>
                </div>
              </div>
            </div>
          </div>
                    ...
                    <c:forEach items="${routeConfigBean.routeInfo.bankCodes }" var="bankCodes">
                      <c:if test="${bankCodes eq (fn:split(m.key,'.')[1]) }">
                        <div class="checkbox-wrap">
                          <input type="checkbox" class="checkbox" name="routeConfigBean.routeInfo.bankCodes" value="${fn:split(m.key,".")[1] }" checked disabled>
                          <label>${m.value}</label>
                        </div>
                      </c:if>
                    </c:forEach>
                      ...
                      <c:forEach items="${map.value }" var="m">
                        <c:forEach items="${routeConfigBean.routeInfo.accountTypes }" var="accountTypes">
                          <c:if test="${accountTypes eq (fn:split(m.key,'.')[1]) }">
                            <div class="checkbox-wrap">
                              <input type="checkbox" class="checkbox" name="routeConfigBean.routeInfo.accountTypes" value="${fn:split(m.key,".")[1] }" checked disabled>
                              <label>${m.value}</label>
                            </div>
                          </c:if>
                        </c:forEach>
                  ...
                  <c:forEach items="${map.value }" var="m">
                    <c:forEach items="${routeConfigBean.routeInfo.cardTypes }" var="cardTypes">
                      <c:if test="${cardTypes eq (fn:split(m.key,'.')[1]) }">
                        <div class="checkbox-wrap">
                          <input type="checkbox" class="checkbox" name="routeConfigBean.routeInfo.cardTypes" value="${fn:split(m.key,".")[1] }" checked disabled>
                          <label>${m.value}</label>
                        </div>
                      </c:if>
                    </c:forEach>
                  ...
                  <c:forEach items="${map.value }" var="m">
                    <c:forEach items="${routeConfigBean.routeInfo.cerTypes }" var="cerTypes">
                      <c:if test="${cerTypes eq (fn:split(m.key,'.')[1]) }">
                        <div class="checkbox-wrap">
                          <input type="checkbox" class="checkbox" name="routeConfigBean.routeInfo.cerTypes" value="${fn:split(m.key,".")[1] }" checked disabled>
                          <label>${m.value}</label>
                        </div>
                      </c:if>
                    </c:forEach>
