		...
		<vlh:root value="dpayRequestExport" url="?" includeParameters="*" configName="defaultlook">
			<table cellpadding="0" cellspacing="1" class="global_tableresult">
				<vlh:row bean="dpayRequestExport">					
					...
					<vlh:column title="订单号" property="flow_no"	 attributes="width='100px'" />
					...
					<vlh:column title="状态" attributes="width='80px'">
						...
						<s:if test="${dpayRequestExport.audit_status=='REMIT_WAIT' }">
							<span style="color: green;">付款审核</span>
						</s:if>
						<s:if test="${dpayRequestExport.audit_status=='REFUSE'}">
							<span style="color: red;">审核拒绝</span>
						</s:if>
						<s:if test="${dpayRequestExport.audit_status=='REMIT_REFUSE' }">
							<span style="color: red;">付款拒绝</span>
						</s:if>
						<s:if test="${dpayRequestExport.audit_status=='PASS' && (dpayRequestExport.status=='WAIT' || dpayRequestExport.status=='HANDLEDING')}">
							<span style="color: #ff9800;">处理中</span>
						</s:if>
						<s:if test="${dpayRequestExport.status=='SUCCESS'}">
							<span style="color: blue;">成功</span>
						</s:if>
						<s:if test="${dpayRequestExport.audit_status=='PASS' && dpayRequestExport.status=='FAILED'}">
