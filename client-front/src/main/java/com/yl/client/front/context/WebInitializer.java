...
import com.yl.client.front.listener.DictLoad;
...
public class WebInitializer implements WebApplicationInitializer {
	...
	public void onStartup(ServletContext servletContext) throws ServletException {
		...
		try {
			...
			environment.getPropertySources().addFirst(new ResourcePropertySource("classpath:/mongodb.properties"));
		} catch (IOException e) {
			logger.error("", e);
		}
		// 管理Spring上下文的生命周期
		servletContext.addListener(new ContextLoaderListener(rootContext));
		servletContext.addListener(new DictLoad());
