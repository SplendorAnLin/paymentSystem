...
public class DictLoad implements ServletContextListener {
	...
	static {
		try {
			prop.load(
					new InputStreamReader(DictLoad.class.getClassLoader().getResourceAsStream("serverHost.properties")));
		} catch (IOException e) {
	...
	public void contextInitialized(ServletContextEvent sce) {
		...
		try {
			...
			try {
				MailUtils.send(MailType.EXCEPTION, "【APP系统】字典加载异常",e.getMessage(), prop.getProperty("mail.from"));
