		...
		<c:forEach items="${accountRecordedDetails }" var="ard">
			<tr align="center">
                ...
                <td>
                    ...
                    <c:if test="${ard.symbol.name() == 'SUBTRACT'}">-<fmt:formatNumber value="${ard.transAmount }" pattern="#.##"/>
                    </c:if>
                </td>
				<%-- <td>
					<div style="vnd.ms-excel.numberformat:#,##0.00">
						<c:if test="${ard.symbol.name() == 'PLUS'}">+</c:if><c:if test="${ard.symbol.name() == 'SUBTRACT'}">-</c:if>${ard.transAmount }
					</div>
				</td> --%>
				<td>
					<div style="vnd.ms-excel.numberformat:#,##0.00">
