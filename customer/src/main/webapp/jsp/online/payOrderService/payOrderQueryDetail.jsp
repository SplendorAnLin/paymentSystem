		...
		<c:if test="${tradeOrder != null }">
			<table cellpadding="0" cellspacing="1" class="global_tabledetail">
				...
				<tr>
					...
					<th width="20%">订单状态</th>
					<td width="30%">
						<c:if test="${tradeOrder.status == 'WAIT_PAY'}">等待付款</c:if>
						<c:if test="${tradeOrder.status == 'SUCCESS'}">成功</c:if>
						<c:if test="${tradeOrder.status == 'CLOSED'}">关闭</c:if>
						<c:if test="${tradeOrder.status == 'FAILED'}">失败</c:if>
					</td>
				</tr>
				...
				<c:forEach items="${payments }" var="pay">
					<tr>
						...
						<!-- B2C, B2B, AUTHPAY, REMIT, WXJSAPI, WXNATIVE, ALIPAY -->
							<dict:write dictId="${pay.payType }" dictTypeId="BF_SHARE_PAYTYPE"></dict:write>
						</td>
