...
<body style="width: 400px; padding:0.2em;">
    ...
    <div class="query-form">
      <form class="validator notification" action="operatorAdd.action" method="post" prompt="DropdownMessage"  data-success="新增成功" data-fail="新增失败">
        <div class="fix">
          <div class="item">
            <div class="input-area">
              ...
              <div class="input-wrap">
                <input type="text" class="input-text" name="operator.username" required checkOperatorPhone trigger='{"checkOperatorPhone": 2}'>
              </div>
            </div>
          </div>
          <div class="item">
          ...
          <div class="item">
            <div class="input-area">
              <span class="label w-90">代付审核密码:</span>
              <div class="input-wrap">
                <input type="password" class="input-text" name="operator.complexPassword" required rangelength="[6, 12]">
              </div>
            </div>
          </div>
          <div class="item">
            <div class="input-area">
              <span class="label w-90">代付审核密码确认:</span>
              <div class="input-wrap">
                <input type="password" class="input-text" name="auditPasswordConfirm" required rangelength="[6, 12]" equalTo="[name='operator.complexPassword'">
              </div>
            </div>
          </div>
          <div class="item">
  ...
  <script>
    ...
    Compared.add('checkOperatorPhone', function(val, params, ele, ansyFn) {
      Api.customer.checkOperatorPhone(val, function(status) { 
    ...
    // todo 增加 获取类型下拉列表api
    Api.customer.operatorType(function(data) {
      // 获取SelectBox组件实例
