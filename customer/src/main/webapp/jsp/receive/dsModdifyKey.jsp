...
<body class="pd-10" onload="changeKey();">
  ...
  <div class="row">
    <div class="title-h1 fix">
      <span class="primary fl">代收商户密钥查询/更新</span>
    </div>
    <div class="content">
      <form action="dsPayKeyUpdate.action" method="post" onsubmit="return checkForm();" style="display: inline-block;">
        <input type="hidden" id="keys" />
        <input type="hidden" id="msg" value="${errorsMsg}">
        <input type="hidden" id="prk" name="receiveConfigInfo.privateCer">
		<input type="hidden" id="puk" name="receiveConfigInfo.publicCer">
        <div class="fix mt-10">
          <div class="item">
            <div class="input-area">
              <span class="label w-90">商户编号:</span>
              <div class="input-wrap">
                <input type="text" class="input-text" id="customerNo" name="receiveConfigInfo.ownerId" value="${sessionScope.auth.customerno}" readonly>
              </div>
            </div>
          </div>
          <div class="item">
  ...
  <script>
    $(document).ready(function() {
      ...
      if(msg == "success"){
        new window.top.Alert("密钥更新成功！", '代收密钥');
      }
      if(msg == "failed"){
        new window.top.Alert("密钥更新失败！", '代收密钥');
      }
    });
    ...
    function changeKey() {
      ...
      if ($.trim(prk.val()) == '' || $.trim(puk.val()) == '') {
        Api.customer.changeDsKeys(customerNo, function(result) {
          if (result == '') {
            new window.top.Alert("您暂未开通代收！", '代收密钥');
            return;
          }
        ...
        Api.customer.dsChangeKeys(function(result) {
          var res = result.split('_');
          var newkey = newPublicKey.html($.trim(res[1])).html();
          newPublicKey.html(newkey.replace(/(\\r|\\n)/g, ''));
          puk.val($.trim(res[1]));
