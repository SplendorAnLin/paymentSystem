...
<body class="bc-lightGray">
	<div class="mobileFrame">
		...
		<form:form class="validator" action="${applicationScope.staticFilesHost}/gateway/again" method="POST" >
				...
				<div class="in row">
					<label class="col-3">手机号:</label> <input class="col-9"
						type="text" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
						  name="phone" value="${phone }"  readonly="readonly" >
				</div>
			</div>
			<div class="input-row">
				<div class="in row">
  ...
  <script>
    ...
    $.addMethod('checkVerificaCode', function(value, element, param, fn) {
      return value == VerificaCode.get(getVerifyUrl());
    }, '验证码错误!');
    $('#verCode').click(function() {
      var element = $(this);
      
      VerificaCode.send(getVerifyUrl(), function(time) {
    ...
    $.addMethod('checkPhone', function(value, element, param, fn) {
		$.ajax({
			...
			function getVerifyUrl() {
				return ('${applicationScope.staticFilesHost}/gateway/sendVerifyCode?phone='+ phone.val());
			}	
	var dialog = $('<div></div>');
	$('.pact').click(function() {
		dialog.html('${content}');
