...
/*	function getSelectInfo(select){
	...
	Select.prototype = {
		init: function(){
			var self = this;
			//
			var selectInfo = self.selectInfo = self.getSelectInfo(self.select);
			self.select.hide();
		...
		getSelectInfo: function(select){
			...
			function getOptionsData(options){
				options.each(function(){
					...
					optionStr += _tpl(self.TPLS.optionHtml, tplDate);
					index++;
				});
			}
			...
			if (select.find('optgroup').length) {
				select.find('optgroup').each(function (index) {
					optionStr += _tpl(self.TPLS.optgroupHtml, {
						index: index,
						label: this.label
					});
					getOptionsData($(this).find('option'));
			...
			// 获取选中
			var selectedOptions = _getOption(select);
			var selectboxHtml = _tpl(self.TPLS.selectboxHtml, {
				style: select.attr('style') || '', 
				textContent: selectedOptions.html() || '',
				value: selectedOptions.val()
			});
			...
			return {
				...
				'top': select.offset().top,
				'selectBox': selectboxHtml,
				'dropdown': _tpl(self.TPLS.dropdownHtml, {
					options: optionStr
				})
			}
		...
		bind: function(){
			var self = this;
			self._selectBox.bind('click', function(){
				self.click();
			});
	...
	$.fn.selectbox = function(opts){
		...
		return this.each(function(i, element){
			...
			var exp =  new Select($(this), opts);
			element.ExpSelect = exp;
