package com.yl.payinterface.core.handle.impl.quick.mipay;
 ...
 * @version V1.0.0
 */
@Service("quickMiPayY1000002Handler")
public class Mipay100002AuthPayHandler implements QuickPayHandler, BindCardHandler{
...
//		map.put("interfaceRequestID", "TD-20170812-100745970921");
	...
	public Map<String, String> query(Map<String, String> map) {
		...
		try {
			...
			if("00".equals(reqMap.get("status"))){
				resParams.put("interfaceCode", map.get("interfaceInfoCode"));
				resParams.put("interfaceRequestID", map.get("interfaceRequestID"));
				resParams.put("businessCompleteType", BusinessCompleteType.AUTO_REPAIR.toString());
				
				resParams.put("tranStat", "SUCCESS");
				resParams.put("amount", reqMap.get("price"));
				...
				if("SUCCESS".equals(reqMap.get("qfStatus"))){
					resParams.put("responseCode", reqMap.get("status") + "-" + reqMap.get("qfStatus"));
					resParams.put("responseMessage", "交易成功,代付成功");
				}else if("FAILED".equals(reqMap.get("qfStatus"))){
					resParams.put("responseCode", reqMap.get("status") + "-" + reqMap.get("qfStatus"));
					resParams.put("responseMessage", "交易成功,代付失败");
				}
			}else{
	...
	public Map<String, String> sendVerifyCode(Map<String, String> params) {
		...
		try {
			...
			// 获取结算信息
			getSettleInfo(inv, interfaceRequest.getOwnerID(), interfaceRequest);
			
			// 保存交易卡历史信息
			quickPayFeeService.saveTransCardHis(params.get("cardNo"), interfaceRequest);
			...
			if(StringUtils.notBlank(params.get("callbackUrl"))){
				...
				inv.setCallbackUrl(properties.getProperty("callBackUrl")+"?order="+interfaceRequest.getBussinessOrderID()+"&");
			}
			inv.setTradeNo(interfaceRequest.getInterfaceRequestID());
	...
	private void getSettleInfo(NewInvoiceData ionvoiceData, String ownerId, InterfaceRequest interfaceRequest){
		TransCardBean transCardBean = customerInterface.findByCustAndAccNo(ownerId, ionvoiceData.getPayCardNo());
		if(transCardBean == null){
			throw new BusinessRuntimeException("3001");
		}
		// 结算账户银行
		ionvoiceData.setBankName(transCardBean.getBankName());
		// 结算账户账号
		ionvoiceData.setCardNo(transCardBean.getAccountNo());
		/// 持卡人姓名
		ionvoiceData.setAccName(transCardBean.getAccountName());
		// 持卡人身份证号码
		ionvoiceData.setAccIdCard(transCardBean.getIdNumber());
		String remark = interfaceRequest.getRemark();
		Map<String, Object> map = null;
		double quickFeeVal, remitFeeVal;
		
		try {
			if (StringUtils.isNotBlank(remark)) {
				map = JsonUtils.toObject(String.valueOf(remark), new TypeReference<Map<String, Object>>(){});
			}
		} catch (Exception e) {
		...
		if (map == null) {
			...
			quickFeeVal = authFee.getFee();
			remitFeeVal = remitFee.getFee();
		} else {
			if (SETTLE_TYPE_FEE.equals(map.get("settleType")) ) {
				try {
					quickFeeVal = Double.valueOf(map.get("quickPayFee").toString());
					remitFeeVal = Double.valueOf(map.get("remitFee").toString());
				} catch (Exception e) {
		...
		// 交易手续费
		ionvoiceData.setDownPayFee(new BigDecimal(AmountUtils.multiply(quickFeeVal, 1000d)).setScale(2, BigDecimal.ROUND_HALF_UP));
		// 结算手续费
		ionvoiceData.setDownDrawFee(new BigDecimal(remitFeeVal).setScale(2, BigDecimal.ROUND_HALF_UP));
	}
