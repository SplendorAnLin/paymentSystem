 ...
 * @since 2017年7月6日
 */
@Service("zhongMaoHandler100003")
public class ZhongMao100003HandlerImpl implements RemitHandler, ChannelReverseHandler {

    private static Logger logger = LoggerFactory.getLogger(ZhongMao100003HandlerImpl.class);
    private static Properties prop = new Properties();

    static {
        try {
            prop.load(ZhongMao100003HandlerImpl.class.getClassLoader().getResourceAsStream("serverHost.properties"));
        } catch (IOException e) {
            logger.error("系统异常:", e);
        }
    }
...
//        map.put("requestNo", "TD-20170904-100192459399"); //1499316452417
        ...
        System.out.println(new ZhongMao100003HandlerImpl().remit(map));
    }
    ...
    public Map<String, String> remit(Map<String, String> map) {
        ...
        nvps.add(new BasicNameValuePair("version", "V3.0"));
        ...
        nvps.add(new BasicNameValuePair("origOrderNo", map.get("orderPayCode")));
        nvps.add(new BasicNameValuePair("origOrderDate", new SimpleDateFormat("yyyyMMdd").format(new Date())));
