...
public class HttpClientUtil {
	...
	public static String doPost(String url, Map<String, String> header,
		...
		try {
			...
			if (header != null && !header.isEmpty()) {
				headerJson = JSON.toJSONString(header,
	 ...
	 * 打印Log
	 */
	private static void printLog(String url, String header, String param,
                                 String result, long start, long end, Exception e) {
		if (logger.isInfoEnabled()) {
			logger.info("调用业务平台:url:[{" + url + "}],header:[{" + header
					+ "}],params:[{" + param + "}],result:[{" + result
					+ "}],start:[{" + start + "}],end:[{" + end
					+ "}],useTime:[{" + (end - start) + "}ms],exception:[{" + e
					+ "}]");
		}
	}
...
//			return HttpClients.custom().setSSLSocketFactory(sslsf).build();
			//return HttpClients.custom().setRedirectStrategy(new DefaultRedirectStrategy()).setSSLSocketFactory(sslsf).build();
	 ...
	 * @return
	 */
	public static String postReq(String requrl, String req, int connTimeOut,
                                 int readTimeOut) {
		try {
