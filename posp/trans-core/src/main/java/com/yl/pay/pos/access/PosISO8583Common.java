...
import com.pay.common.util.ISO8583.*;
...
import com.yl.pay.pos.util.codecs.common.tlv.BerTlvDecoder;
import com.yl.pay.pos.util.codecs.common.tlv.TLVDecoder;
...
public class PosISO8583Common {
    ...
    private static final String TLV_VALUE_ENCODING = "GBK";
	...
	static {
		...
		ItemTbl.put(new FieldId(47), new Field8583(47,160,LengthType.LLLVAR, DataType.S,EncodeType.ASCII));		//....
		...
		ItemTbl.put(new FieldId(51), new Field8583(51,500, LengthType.LLLVAR, DataType.S,EncodeType.ASCII));	//个人标识码数据
		...
		ItemTbl.put(new FieldId(55), new Field8583(55,1024,LengthType.LLLVAR,  DataType.S,EncodeType.ASCII));	//附加金额
		...
		ItemTbl.put(new FieldId(60), new Field8583(60,017,LengthType.LLLVAR, DataType.N,EncodeType.BCD));		//自定义域
		...
		ItemTbl.put(new FieldId(62), new Field8583(62,1024,LengthType.LLLVAR, DataType.S,EncodeType.ASCII));		//自定义域
		...
		ItemTbl.put(new FieldId(633),new Field8583(633,30,LengthType.LLVAR, DataType.S,EncodeType.ASCII));		//自定义域2
	...
	public Object Decoding(byte[] data, int len, UnionPayBean unionPayBean) throws UnsupportedEncodingException {
		...
		for (int i = 0; i < maxfields; i++) {
			...
			for (int j = 0; j < 8; j++) {
				...
				if (fld == 1) {
					...
					try {
						if(51==fld){
//							int byteLength=byteLength(fld,data, offset, len);
//							byte[] by=new byte[byteLength];
//							System.arraycopy(data, offset+2, by, 0, byteLength);
//							
//							TLVDecoder decoder = new BerTlvDecoder();
//							TLVObject[] tlvs = decoder.decode(by);
//							TLVObject tlv = tlvs[0];
//							String value51 = new String(tlv.getByteValue(),TLV_VALUE_ENCODING);
//							copyObject = putBeanbyItem(fld, value51, copyObject);
//							log.info("field[" +fld+":"+ tlv.getTag() + "]: ["+((UnionPayBean)copyObject).getCurrencyCodeCardholder()+"] ");
//							offset += byteLength + 2;
//							len -= (byteLength + 2);
//							continue;
							int byteLength=byteLength(fld,data, offset, len);
							...
							Map<String, String> tlvsMap = new HashMap<String, String>();
							TLVDecoder decoder = new BerTlvDecoder();
							TLVObject[] tlvs = decoder.decode(by);
							for(int k = 0; k<tlvs.length; k++)
							{
								TLVObject tlv = tlvs[k];
								tlvsMap.put(tlv.getTag(), new String(tlv.getByteValue(),TLV_VALUE_ENCODING));
								log.info("field[" +fld+":"+ tlv.getTag() + "]: ["+new String(tlv.getByteValue())+"] ");
							}
							
							copyObject = putBeanbyItem(fld, tlvsMap, copyObject);
							offset += byteLength + 2;
							len -= (byteLength + 2);
							continue;
						}else{
						...
						if(fld==63){
							putBeanbySpecialItemTemporary(fld, field.getValue(), copyObject);
						}else{
							throw new POSISO8583Exception(e);
						}
					}	
				log.info("field[" + fld + "]: ["+ 
						field.getValue() + "," + 
				...
				if(fld==55){
					TlvUtils.builderTlvMap(field.getValue());
				}
				if (fld == 52) {//
					len -= 8;
					offset += 8;
	...
	public byte[] Encoding() throws UnsupportedEncodingException {
...
//					TLVObject[] tlvs = new TLVObject[1];
//					
//	    			tlvs[0] = new TLVObject(Constant.TAG_SETTLE_CARD,);
//	    			byte[] keybyte = (new BerTlvEncoder()).encode(tlvs);
//					String keylen = ISO8583Utile.int2char3(keybyte.length);
//					byte[] lenbyte = ISO8583Utile.string2Bcd(keylen);// 域长度BCD码
//					byte[] bytevalue = new byte[keybyte.length + 2];
//					System.arraycopy(lenbyte, 0, bytevalue, 0, 2);
//					System.arraycopy(keybyte, 0, bytevalue, 2, keybyte.length);
//					itembytes = bytevalue;
					byte[] bytes=ISO8583Utile.hexStringToByte(itm.getValue()); 
					String keylen =ISO8583Utile.int2char3(bytes.length);
					byte[] lenbyte =ISO8583Utile.string2Bcd(keylen);// 域长度BCD码
					byte[] bytevalue = new byte[bytes.length + 2];
					System.arraycopy(lenbyte, 0, bytevalue, 0, 2);
					System.arraycopy(bytes, 0, bytevalue, 2, bytes.length);
					itembytes = bytevalue;
				}else {
				...
				log.info("field[" + i + "]: ["+ 
						 ...
						 itm.getDataType() + "] ");
				if(i==55){
					TlvUtils.builderTlvMap(itm.getValue());
				}
			}			
		}
		...
		//管理类交易不需要生成MAC
		if ("0810,0830,0510,0330,0710".indexOf(messageType)<0) {
...
//				mac = DesUtil.desEncrypt(xforesult,"04DCCB1FEC5B8C67").substring(0, 8);
				mac= EsscUtils.createPosMac("pos."+getField(41).getValue()+".zak", makeMacString);
			} catch (Exception e) {
				try {
	...
	private static Field8583 DecodingItem(int fld,
		...
		if (LengthType.APPOINT.equals(itmdef.getLengthType())) {// 定长
			if (DataType.N.equals(itmdef.getDataType())
				...
				if(itmdef.getId()==23){
					str = str.substring(str.length()-maxlen, str.length());
				}else{
...
//					str = ISO8583Utile.bytesToHexString(itemValue);
	...
	public static Object putBeanbySpecialItem(int fld, String value,
		...
		if (fld == 57 || fld == 59 ||fld == 60 || fld == 61 || fld == 63) {// 目前只考虑主域和子域编码一样的情况****
			...
			for (int i = fld * 10 + 1; i < fld * 10 + 6; i++) {
				...
				if (field8583 != null) {// 域的长度要大于子域的长度
					if (LengthType.APPOINT.equals(field8583.getLengthType())) {// 定长
						...
						if (LengthType.LLVAR.equals(field8583.getLengthType())) {// 两位变长
							newfld = Integer.parseInt(ISO8583Utile.bytesToHexString(realValue.substring(
									startlen, startlen + 1).getBytes()));
								copyObject = putBeanbyItem(i, realValue.substring(
										startlen + 1, startlen + 1 + newfld),
										copyObject);
								startlen += newfld + 1;
	...
	public static Object putBeanbySpecialItemTemporary(int fld, String value,
			Object object) {// 目前这三个域比较特殊
		Object copyObject = object;
		int newfld = 0;
		int startlen = 0;
				...
				if (field8583 != null) {// 域的长度要大于子域的长度
					if (LengthType.APPOINT.equals(field8583.getLengthType())) {// 定长
						...
						if (LengthType.LLVAR.equals(field8583.getLengthType())) {// 两位变长
							...
							if(fld==63&&i>=632){  //针对于华智融的基站取负的问题
								break;
							}
								copyObject = putBeanbyItem(i, realValue.substring(
										startlen + 1, startlen + 1 + newfld),
	...
	public static Object putBeanbyItem(int fld, Object value, Object object) {
		Object copyObject = object;
		// 获得对象的类型
...
//		}
	...
	public  int byteLength(int fld, byte[] data, int offset, int len){
		Field8583 itmdef = null;
		try{
			itmdef = getItemDefine(fld);
		}catch(ICNotSupportException e){
		...
		String str = "";
		int bytelen=0;
		if(LengthType.LLLVAR.equals(itmdef.getLengthType())){
			byte[] itemValuelen = new byte[2];// 域长度BCD
			System.arraycopy(data, offset, itemValuelen, 0, 2);
			str = ISO8583Utile.bcd2String(itemValuelen, false);
			bytelen = Integer.parseInt(str);// 域真实长度
