...
public class CcbISO8583 {
	private static final Log log = LogFactory.getLog(CcbISO8583.class);
	...
	public final static String CONTENT_CCB_TPUD = "6000040000";				// 生产	
	
	public final static int LEN_CCB_MESSAGE = 6;							// 报文头长度
	public final static String CONTENT_CCB_MESSAGE = "600100310017";	
	...
	static {
		...
		domainDefPool.put(new FieldId(47), new Field8583(47, 2 ,  LengthType.LLLVAR, DataType.N, EncodeType.BCD));	// (作为是否D0结算标识)
	 ...
	 */
	public void putItem(int d, String data) throws CCBTransException {
		// >=600为扩展域,不判断 
		if (d<570) {
			log.info(d);
		...
		if (itmdef == null)
			throw new CCBTransException(
					CCBTransException.FIELD_NOT_DEFINED_IN_8583);

		// 检查数据的长度是否合法
		if (data.length() > itmdef.getLength())
			throw new CCBTransException(
					CCBTransException.ERROR_DATA_TYPE);
	...
	public Field8583 putValue(int i) throws CCBTransException {// 对于有子域的域，把子域的值拼接好后放入到对应的域中
		Field8583 itm;
	...
	public byte[] Encoding() throws  Exception {
		...
		if (!Constant.NOMACTYPE_SIGNIN.equals(messagetype)
				&& !Constant.NOMACTYPE_SIGNOFF.equals(messagetype)) {
...
//			String macResult = DesUtil.desEncrypt(xforesult,"C45DA4CD1007CEBC").substring(0, 8);
			String macResult =EsscUtils.createPosMac("bank."+Constant.INTERFACE_CODE+"_"+getItem(41).getValue()+".zak", makeMacString);
...
//					}
	...
	private static UnionPayBean putPropertyVaule(UnionPayBean resBean,
		...
		Map<Integer, String> fields = CcbParamMapping.ccbUnionMap;
	...
	public UnionPayBean Decoding(byte data[], int dataLen)
		...
		if (!CcbParamMapping.getMtpResult(message)) throw new Exception(); 	// 判断返回消息类型是否正确
		...
		int charlen = 0;
		CcbISO8583 ccbResponse = new CcbISO8583(); 
		Field8583 ins = null;
		...
		if (dataLen != 0) {
			if (StringUtil.notNull(resBean.getResponseCode()) && !Constant.BANK_RESP_CODE_00.equals(resBean.getResponseCode())) {
				throw new CCBTransException(CCBTransException.ERROR_PARSE_ITEM_LENGTH);
			} else {
				throw new Exception();
			}
		}
	...
	public static void main(String[] args) throws Exception {
		...
		CcbISO8583 ccb = new CcbISO8583();
		try{
			byte [] res_byte=ISO8583Utile.hexStringToByte("");
