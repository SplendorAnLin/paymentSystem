...
<html>
	<head>
		...
		<!--页面样式-->
		<link rel="stylesheet" href="${pageContext.request.contextPath}/css/assest/css/home/card_public.css?author=zhhupan&v=1128" />
			<!--页面自适应js-->
	...
	<body style="background: white;">
		...
		<section>
			...
			<a href="toAddcard">
			<div class="card_bg bg_gray">
				<div class="fon_28 text_center ver_200"><img class="w-50 po_re re_top_10 " src="${pageContext.request.contextPath}/image/public/mycenter/add.png" /> 添加银行卡</div>
			</div>
		</a>
			</section>
		...
		$(function() {
          /***银行卡添加节点操作start**/
			function add_card(bankNames, cardType,cardAlias, bankNOs,bankIco,bgcolor,cardAttr) {
				...
				var bankName = $("<p>").addClass("fon_30").append(bankNames);
				...
				card_line_s.append(bankNO).append(deleteimg);
			...
			$.ajax({
				...
				success: function(data) {
					if(data.responseData.cardList.length > 0) {
						for(var i = 0; i < data.responseData.cardList.length; i++) {
						    var cardType;
                            var bankIco,bgcolor,bankCode;
                            var bankName=data.responseData.cardList[i].bankName;
                            var cardAlias=data.responseData.cardList[i].cardAlias;
                            var accountNo= data.responseData.cardList[i].accountNo;
                            var cardAttr= data.responseData.cardList[i].cardAttr;
                            bankCode=data.responseData.cardList[i].bankCode;
						    if(data.responseData.cardList[i].cardType=="DEBIT"){cardType="借记卡"}
						    else if(data.responseData.cardList[i].cardType=="CREDIT"){cardType="信用卡"}
                                $.ajax({
									type:"get",
									data:"",
                                    async: false,
                            ...
                            var cards = add_card(bankName, cardType,cardAlias,accountNo ,bankIco,bgcolor,cardAttr);
                            $(".hea").append(cards);
						}
