                ...
                setTimeout(function() {
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            'currentPage': page,
                            'startSuccessPayTime': $("#date-selector-input").val(),
                            'endSuccessPayTime': $("#date-selector-inputs").val()
                        },
                        success: function(data) {
                            ...
                            for(var i = 0; i < data.result.length; i++) {
                                var dates = new Date(data.result[i].createTime.time);
                                ...
                                data.result[i].createTime.time = year + "-" + month + "-" + da + "   " + hour + ":" + minute + ":" + second;
                                var texts='';
                                var clazz = "";
                                if(data.result[i].status=="SUCCESS"){
                                    ...
                                    texts="关闭";
                                    clazz= "#FF0000";
                                } else if(data.result[i].AUDIT_STATUS=='FAILED'){
                                    texts="失败";
                                    clazz= "#FF0000";
                                }else if(data.result[i].AUDIT_STATUS=='WAIT_PAY' ){
                                    texts="等待支付";
                                    clazz= "#FF9800";
                                }
                                data.result[i].amount=parseFloat(data.result[i].amount);
								var money=data.result[i].amount.toFixed(2);
                                var	vation = '<a target="_top" href="payOrderDetail?code='+data.result[i].code+'">'+
                                    '<li><div class="img_paymethod"><img src="image/logo.png" /> </div>' +
                                    '<div class="pay_data">' +
                                    '<p class="font_paymethod">' + data.result[i].code + '</p>' +
                                    '<p class="pay_time">' + data.result[i].createTime.time + '</p>' +
                                    '</div>' +
