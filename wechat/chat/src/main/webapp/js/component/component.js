    ...
    <div class="message-wrap">\
      <div class="message-content">\
        <div class="message-html">\
          ...
          <div class="conten-wrap ib h-p-100">\
            <span class="middle"></span>\
            <div class="txt-middle ib">\
            </div>\
          </div>\
        </div>\
...
(function() {
  ...
  // 关闭对话框
  MyDialog.prototype.close = function(callback, forced) {
    var self = this;
    var options = self.options;
    // 禁用状态下不允许关闭
  ...
  MyDialog.prototype.createBtns = function(btns) {
    ...
    if (type === 1) {
      btnWrap.removeClass('Alert-footer').addClass('btn-footer');
    }
  ...
  // 对话框加载动画
  MyDialog.prototype.loading = function(loading) {
    // 创建等待元素
    ...
    if (loading) {
      if (self.disabled)
        return;
      $('.loadImg', loadElement).css('background-image', 'url(' + MyDialog.loadImg + ')');
      if (self.content)
  ...
  // 封装 Confirm
  var Confirm = function(element, title, onConfirm, onCancel, noPadding) {
    // 默认参数
    var option = {
      // 显示目标, 一个jQuery对象
      target: (typeof(element) !== 'string') ? element : $('<div><p>' + element + '</p></div>'),
...
(function() {
   ...
   */
  var switchText_html = '\
    <div class="toggle-switch switch-text" {{checked}}  {{disabled}} >\
  ...
  SwitchText.prototype.create = function() {
    ...
    // 生成虚拟checkbox
    var html = utils.tpl(switchText_html, {label: label.text() || ''});
...
(function() {
  ...
  FormValidator.prototype.element = function(ele, callback, trigger) {
    ...
    Compared[struct.testMethod](struct.rules, function(status) {
      ...
      if (status.status !== EnumStatus.asyn) {
        if (ele.get(0).ValidtorComplete)
          ele.trigger('FormValidator_Status', status);
      }
    ...
    var validatorElements = $();
     ...
     var judg = function() {
      ...
      validatorElements.each(function() {
        ...
        // console.warn(this, '元素验证结果', this.enumstatus);
        if (status == 'false') {
          fail = true;
          return;
        } else if (status == 'asyn') {
      ...
      if (fail) {
        callback({ status: EnumStatus.fail});
      } else if (asyn) {
        callback({ status: EnumStatus.asyn});
      } else {
        callback({ status: EnumStatus.ok});
      }
